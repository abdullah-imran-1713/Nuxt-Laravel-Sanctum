...
// const { $useIFetch } = useNuxtApp();
// const options = {
//     method: "POST",
//     credentials: 'include',
//   };
//   const { data, error: error2 } = await $useIFetch("/register", options);
//   loading.value = false;
//   console.log(data.value, error2.value);
...
const handleSubmit = async () => {
  ...
  try {
    const response = await $useIFetch('/register', { 
      method: 'POST',
      body: { 
        name: name.value,
        email: email.value,
